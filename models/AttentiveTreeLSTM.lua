...
function ChildSumTreeLSTM:__init(config)
    ...
    -- attention module
    self.attention_module = self:new_attention_module()
end
...
function ChildSumTreeLSTM:new_attention_module()
    local input = nn.Identity()()
    local seq_copies = nn.Identity()()
    local t_input = nn.JoinTable(1)(input)
    local M = nn.Tanh()(nn.CAddTable() {
        nn.Linear(self.mem_dim, self.mem_dim, false)(t_input),
        nn.Linear(self.mem_dim, self.mem_dim, false)(seq_copies)
    })
    local temp = nn.Linear(self.mem_dim, 1)(M)
    local attention_weights = nn.Transpose({1,2})(nn.SoftMax()(nn.Transpose({1,2})(temp)))
    local h_weighted_sum = nn.Reshape(self.mem_dim)(
        nn.MM(true, false)({ t_input, attention_weights })
    )
    return nn.gModule({input, seq_copies}, {h_weighted_sum})
end
...
function ChildSumTreeLSTM:forward(tree, inputs, seq_rep)
    self:_forward(tree, inputs)
    local queue = {}
    table.insert(queue, tree)
    local hidden_states = {}
    table.insert(hidden_states, tree.state[2]:reshape(1, self.mem_dim))
    while #queue ~= 0 do
        local this_tree = queue[1]
        table.remove(queue, 1)
        for i = 1, this_tree.num_children do
            table.insert(queue, this_tree.children[i])
            table.insert(hidden_states, this_tree.state[2]:reshape(1, self.mem_dim))
        end
    end
    local seq_copies = torch.zeros(inputs:size(1), self.mem_dim)
    for i = 1, inputs:size(1) do
        seq_copies[i] = seq_rep
    end
    local weighted_hidden = self.attention_module:forward({hidden_states, seq_copies})
    return weighted_hidden
end

function ChildSumTreeLSTM:_forward(tree, inputs)
    for i = 1, tree.num_children do
        self:_forward(tree.children[i], inputs)
    end
...
function ChildSumTreeLSTM:forward_copy(tree, inputs)
    local loss = 0
    for i = 1, tree.num_children do
...
function ChildSumTreeLSTM:backward(tree, inputs, seq_rep, grad)
    local grad_inputs = torch.Tensor(inputs:size())
    self:_backward(tree, inputs, seq_rep, grad, grad_inputs)
    return grad_inputs
end

function ChildSumTreeLSTM:_backward(tree, inputs, seq_rep, grad, grad_inputs)
    ...
    local tree_grads = self.attention_module:backward({hidden_states, seq_copies}, grad[2])[1]
    local root_grads = tree_grads[1]
    local composer_grad = tree.composer:backward(
        {inputs[tree.idx], child_c, child_h},
        {grad[1], root_grads + output_grad})
    ...
    for i = 1, tree.num_children do
        self:_backward(tree.children[i], inputs, seq_rep, {child_c_grads[i], child_h_grads[i]}, grad_inputs)
    end
end
